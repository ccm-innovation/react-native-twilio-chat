...
class AccessManager {
  constructor(token) {
    ...
    TwilioAccessManager.accessManagerWithToken(this.token).then(
      parseManager.bind(this)
    );
  }
  ...
  updateToken(newToken) {
    return TwilioAccessManager.updateToken(newToken).then(
      parseManager.bind(this)
    );
  }

  getTokenExpirationDate() {
    return TwilioAccessManager.getTokenExpirationDate();
  }

  getToken() {
    return TwilioAccessManager.getToken();
  }

  isTokenExpired() {
    return TwilioAccessManager.isTokenExpired();
  }
