  ...
  constructor(token, synchronizationStrategy, initialMessageCount) {
    // properties
        ...
        this._channelMessageChangedSubscription = NativeAppEventEmitter.addListener(
            ...
            ({channelSid, message}) => {
                if (this.onMessageChanged) this.onMessageChanged({
                    channelSid, 
        ...
        this._channelMessageDeletedSubscription = NativeAppEventEmitter.addListener(
            ...
            ({ channelSid, message }) => {
                if (this.onMessageDeleted) this.onMessageDeleted({
                    channelSid, 
                    message: new Message(message, channelSid),
                });
            },
    ...
    getUserChannels() {
        return TwilioIPMessagingChannels.getUserChannels()
        .then(({ sid, paginator }) => new ChannelPaginator(sid, paginator));
    }

    getPublicChannels() {
        return TwilioIPMessagingChannels.getPublicChannels()
        .then(({ sid, paginator }) => new ChannelPaginator(sid, paginator));
    }
