...
class Client {
    constructor(accessManager, synchronizationStrategy, initialMessageCount) {        
        ...
        this._channelAddedSubscription = NativeAppEventEmitter.addListener(
            ...
            (channel) => {
                if (this.onChannelAdded) this.onChannelAdded(channel)
            }
        )
        ...
        this._channelChangedSubscription = NativeAppEventEmitter.addListener(
            ...
            (channel) => {
                if (this.onChannelChanged) this.onChannelChanged(channel)
            }
        )
        ...
        this._userInfoUpdatedSubscription = NativeAppEventEmitter.addListener(
            ...
            ({updated, userInfo}) => {
                ...
                if (this.onUserInfoUpdated) this.onUserInfoUpdated({updated, userInfo})
            }
        )
    ...
    createChannel(options) {
        ...
        for (var key in options) {
            ...
            switch(key) {
                case 'friendlyName':
                    newKey = 'TWMChannelOptionFriendlyName'
                    break
                case 'uniqueName':
                    newKey = 'TWMChannelOptionUniqueName'
                break
                case 'type':
                    newKey = 'TWMChannelOptionType'
                break
                case 'attributes':
                    newKey = 'TWMChannelOptionAttributes'
                break
            }
            parsedOptions[newKey] = options[key]
        }
        TwilioIPMessagingChannels.createChannelWithOptions(parsedOptions)
        .then((channel) => {
            return new Channel(channel)
        })
    }
