...
class Client {
        ...
        this._synchronizationStatusChangedSubscription = NativeAppEventEmitter.addListener(
            ...
            (status) => {
                ...
                if (status == Constants.TWMClientSynchronizationStatus.Completed) {
                    ...
                    .then(() => TwilioIPMessagingChannels.allObjects())
                    // .then((channels) => {
                    //     this.channels = channels.map((channel, index) => {
                    //         this._channelSids[channel.sid] = index
    ...
    createChannel(options) {
        TwilioIPMessagingChannels.createChannelWithOptions(options)
        .then((channel) => {
            return new Channel(channel)
        })
    }
