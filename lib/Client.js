...
class Client {
    ...
    initialize() {
        ...
        .then(() => {
            return TwilioIPMessagingClient.createClient(this._properties)
            .then((client) => {
                if (client) {
    ...
    getChannelByUniqueName(uniqueName) {
        return TwilioIPMessagingChannels.getChannelByUniqueName(uniqueName)
        .then((channel) => {
            return new Channel(channel)
        })
    }
    ...
    createChannel(options) {
        ...
        TwilioIPMessagingChannels.createChannel(parsedOptions)
        .then((channel) => {
            return new Channel(channel)
        })
    }
