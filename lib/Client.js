...
const { TwilioChatClient, TwilioChatChannels } = NativeModules;
...
class Client {
  constructor(initialToken, synchronizationStrategy, initialMessageCount) {
    ...
    this._clientConnectionStateChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      state => {
        if (this.onClientConnectionStateChanged)
          this.onClientConnectionStateChanged(state);
      }
    );
    ...
    this._synchronizationStatusChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      status => {
        ...
        if (this.onSynchronizationStatusChanged)
    ...
    this._channelMemberJoinedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberJoined)
          this.onMemberJoined({
            channelSid,
            member: new Member(member, channelSid)
          });
      }
    );
    ...
    this._toastReceivedChannelSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, messageSid }) => {
        if (this.onToastReceived)
          this.onToastReceived({ channelSid, messageSid });
      }
    );
    ...
    this._userUpdatedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ updated, user }) => {
        ...
        if (this.onUserUpdated)
          this.onUserUpdated({ updated, user: this.user });
      }
    );
  ...
  initialize() {
    return TwilioChatClient.createClient(
      this.initialToken,
      this._properties
    ).then(client => {
      if (client) {
  ...
  getUserChannels() {
    return TwilioChatChannels.getUserChannels().then(
      ({ sid, type, paginator }) => new Paginator(sid, type, paginator)
    );
  }

  getPublicChannels() {
    return TwilioChatChannels.getPublicChannels().then(
      ({ sid, type, paginator }) => new Paginator(sid, type, paginator)
    );
  }
  ...
  getChannel(sidOrUniqueName) {
    return TwilioChatChannels.getChannel(sidOrUniqueName).then(
  ...
  createChannel(options) {
    ...
    return TwilioChatChannels.createChannel(parsedOptions).then(
      channel => new Channel(channel)
    );
  }
