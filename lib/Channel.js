...
class Channel {
    constructor(props) {
        ...
        this._channelChangedSubscription = NativeAppEventEmitter.addListener(
            ...
            (channel) => {
                console.log('lib channel changed = ', channel)
    ...
    getMessages(count = 10) {
        // console.log('getting messages in ip messaging lib', arguments, this.sid, count)
        //this is needed for android, with out this the app will go into an infinite loop with the warning
        // Messaging lib not ready, retrying
